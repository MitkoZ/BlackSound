...
namespace BlackSound.Views
{
    ...
    //the ordinary user can CRUD playlists + more
    {
        ...
        protected override List<Playlist> GetListView(BaseRepository<Playlist> baseRepo)
        {
            ...
            List<UsersPlaylists> usersPlaylistsDb = usersPlaylistsRepo.GetAll(userPlaylist => userPlaylist.UserId == AuthenticationService.LoggedUser.Id); //playlists created by me or shared with me
            List<int> playlistsIds = new List<int>();
            foreach (UsersPlaylists userPlaylist in usersPlaylistsDb) //gets only the ids of the playlists for the current user
            {
        ...
        {
            return new PlaylistsRepository(Constants.PlaylistsDirectory);
        }
